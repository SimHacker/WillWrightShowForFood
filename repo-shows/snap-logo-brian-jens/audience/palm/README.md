# Palm — TicketPR (Snap! / Logo pair show) 🐒✋

*Sniff:* [`questions.md`](questions.md) (readable for guests) · [`questions.yml`](questions.yml) (orchestrator SSOT) · canonical Palm: [`characters/palm/`](../../../../characters/palm/README.md)

**At this show:** Palm read Theo the Logo Turtle into his [worm field notebook](https://github.com/SimHacker/moollm/blob/main/examples/adventure-4/pub/stage/palm-nook/study/palm-on-worms-fieldnotes.md) after Snap!Con. He wants Brian and Jens to settle **rings as quote**, the **honest SICP/Scheme scorecard** (macros partial?), and whether the **gray ring gap** is deliberate pedagogy or accident.

**Lead question:** *"Did you design the ring to be visible quote for kids who will never type an apostrophe?"*

**Controversy spine:** Brian **for** dynamic binding (Logo decades) — Palm asks arguments for/against; what Snap! actually ships (lexical? `of`? RISC vs Rube Goldberg?). See question id 3.

**Morphic beat:** Jens on Self → Lively Kernel → Morphic.js → Snap! — and why MVC got cargo-culted while Morphic didn't (question id 10).

**Warm beat:** Jens on SAP supporting Kay's CDG, supporting Snap!, openSAP/Young Thinkers — question id 8.

**Heart beat:** Brian on **no grades** — the seventh-grade algebra teacher who taught friendship instead of algebra; grading spoils the teacher-student relationship; teaching as Martin Buber's I–Thou dialogue, ways to lead lives together (question id 12). No time limit on this one.

**Memorial beat:** Both on people they miss from Logo/blocks/BJC/Snap!Con — lessons still worth learning (question id 11). Framed after the design controversies: harmonious disagreement, not who won.

**Technical spine:** [Metaprogramming digest](https://github.com/SimHacker/WillWrightShowForFood/blob/main/characters/brian-harvey/sources/snap-macros-metaprogramming.md)

Full tree: [`questions.md`](questions.md) — twelve questions, controversy → gratitude → heart → memorial → silly. Every node carries metadata and **Zap** laser links as absolute URLs.

↑ [audience](../README.md) · [pair show](../../README.md) · [Brian](../../../../characters/brian-harvey/) · [Jens](../../../../characters/jens-monig/)

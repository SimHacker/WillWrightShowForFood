# Design-Based Learning × open SimCity

Show seed — Doreen Nelson on DBL, Maxis curriculum guides, and Micropolis for classrooms.

**Show:** [`SHOW.yml`](SHOW.yml) · **Guest:** [`characters/doreen-nelson/`](../../characters/doreen-nelson/README.md)

## Logline

The educator who taught the whole curriculum by **building cities first** — and wrote **Maxis SimCity teacher guides** — meets the **open Micropolis** fork Don cleared for OLPC. What courseware should ship with View Source?

## Segments (draft)

1. **DBL on air** — backwards thinking from model cities to any subject
2. **Dec 2006 thread** — [sourced OLPC response](../../characters/doreen-nelson/sources/2006-12-21-olpc-curriculum-guide/olpc-response.md)
3. **Free the guides** — Leslie Stoltz co-authorship, EA rights, classroom use stories
4. **Micropolis Home** — scripting, spreadsheets, Sugar lineage vs 2026 tools
5. **Constructionist reunion** — Papert, physical models, digital microworlds

## Related

- [`walter-bender-olpc/`](../walter-bender-olpc/SHOW.yml) — OLPC cast segment
- [`2006-12-simcity-olpc-introducing-the-players/`](../../characters/will-wright/sources/2006-12-simcity-olpc-introducing-the-players/README.md)
- [`remembering-the-constructionists`](../remembering-the-constructionists.yml)

↑ [`../README.md`](../README.md)

...
// Rung-1 acceptance criterion, not yet met: the boot's free-list loop reads
// increment/limit cells at 12161/12212, above the .oct image's top (11741).
// Under investigation against the listing; todo keeps the criterion in the
// suite without failing the build.
test("acceptance: SYMELEC boots and issues IDLA", { todo: true }, () => {
	const octPath = new URL(
		"../../../characters/heinz-lemke/sources/pixie-assembler-listing-1972/symelec.oct",
		import.meta.url,
	);
	const cpu = new Pdp7({ coreWords: 8192 });
	const range = loadOct(cpu, readFileSync(octPath, "utf8"));

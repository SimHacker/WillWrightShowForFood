# Ideas to explore with Richard Bartle 👤

*Conversation hooks for a Repo Show — **Don's proposed topics**, each grounded in Richard's
public work and documented connections to this repository. Things Don would love to follow
**with** Richard Bartle; not quotes, not claims about what they think.*
[Portrayal standards](../../schemas/portrayal-standards.md) · invitation guest · consent not_yet_asked

## What Richard has done

Richard Bartle — co-creator of MUD1 (1978) and originator of the Bartle taxonomy of player types; author of *Designing Virtual Worlds*. (Incarnated with his explicit permission.)

## The hooks

### 1. Show seed: `repo-shows/richard-bartle/`
walk the repo on air and build from the seed.

### 2. Will cites Bartle on camera (Dev Diaries Dec 2025)
Will Wright **name-checks the Bartle taxonomy** in the same interview where he describes Proxi
as a software heirloom. Don's capture:
[`will-wright/sources/2025-12-will-wright-dev-diaries/bartle-player-types.yml`](../will-wright/sources/2025-12-will-wright-dev-diaries/bartle-player-types.yml)

**Show beat:** Will (on tape) → Bartle (consented, self-authoring) → taxonomy vs Sims motives vs
Proxi family perspectives. Existing draft questions in
[`audience/richard-bartle/questions.yml`](../../repo-shows/will-wright-premiere/audience/richard-bartle/questions.yml).

### 3. Essex assignment bloopers — ChatGPT era (Facebook Jul 2026)
Bartle marks game-design work at **University of Essex** and publishes anonymised out-takes once
students' tenure ends (waits a year for resits). Lines are "students or ChatGPT" — e.g. *engorges
players to interact*, *dating nuclear bombs*, *whip out the entire group*, *danger lurks around
every cornea*. Don replied on Facebook: **"+4 Stamina Fleshlight of Holding"** on the engorges
line; face-eating-leopard butcher joke on *the face of humanity hangs in the balance*.

**Show beat:** Educators track — MUD2 critical-play assignments vs what autocomplete submits;
Micropolis/repo-show as antidote to slop. Warm Don↔Bartle public thread before invite.

Digest: [`sources/2026-07-bartle-essex-assignment-bloopers-facebook.yml`](sources/2026-07-bartle-essex-assignment-bloopers-facebook.yml)

## Sources (public)

- [`invitation.md`](invitation.md)
- Show seed: [`repo-shows/richard-bartle/`](../../characters/richard-bartle/)
- [`CHARACTER.yml`](CHARACTER.yml)

# Captions — 2006 Long Now (Eno + Will)

**Not started.** YouTube auto-captions garble names and cross-talk (Eno/Will/Stewart/audience Q&A).

When Don proofreads:

1. Add `captions/<clip_id>.vtt` here (clip-relative times, speaker labels).
2. Run `extract-clip.py` against a local master in `~/GroundUp/repo-show/`.
3. Upload `.vtt` with every YouTube publish.

Policy: [captions](../../../../../process/post-production/captions.yml)

↑ [short form clips](../short-form-clips.yml)

# Will's Russian Space Junk

*Will Wright is a longtime collector of real **Soviet/Russian spaceflight hardware** — flown
and ground-test control panels, navigation instruments, hand controllers, hatches, and what
appears to be a **space monkey seat**. These photos were taken by **Don Hopkins** at Will's home in **Oakland**.
Captions describe what is visibly in each frame; the Cyrillic labels are transcribed/translated where legible. Corrections welcome.*

## The Globus (IMP) navigation instrument

The standout pieces are **Globus IMP** units — the famous electromechanical orbital-position
indicator from Soyuz/Vostok-era spacecraft. A motor-driven globe rotates under a fixed reticle
to show the craft's position over Earth, with dials for orbit **ПЕРИОД** (period), **ВИТКИ**
(orbit count), and **УГОЛ ПОСАДКИ** (landing angle).

![Globus IMP — close-up of the rotating Earth globe under the reticle](russian-globus-imp-globe-closeup.png)

Close-up of the rotating Earth globe under the cross-hair reticle.

![Globus IMP panel — orbital period dial reading 86.79](russian-globus-imp-panel-period.png)

A Globus instrument panel: **ПЕРИОД** (period) **86.79**, latitude/longitude scales (**Ш / Д**),
**ВИТКИ** (orbits), the **ОРБИТА** crank, and **УГОЛ ПОСАДКИ** (landing angle).

![Globus IMP panel — landing-site indicator (МЕСТО ПОСАДКИ)](russian-globus-imp-panel-landing.png)

Another Globus panel with the **МЕСТО ПОСАДКИ** (landing site) indicator lit, **ВИТКИ 330.4**.

## Control & command panels

![Soyuz-style command keyboard (КСП ЛЕВ) with numbered enable/disable buttons](russian-ksp-button-panel.png)

A command panel (**КСП ЛЕВ** — left command signaling panel) with rows of Cyrillic-labeled
function keys over numbered **ВКЛ / ОТКЛ** (on/off) buttons 1–18.

![Close-up of Cyrillic command buttons (ПИТАНИЕ, ЗАПРЕТ, ВЫБОР)](russian-control-buttons-closeup.png)

Detail of the labeled buttons — **ПИТАНИЕ** (power), **ЗАПРЕТ** (inhibit), **ВЫБОР** (select),
**СПУСК** (descent), **ОСК**, **ИСК**, **РУЧНОЕ СБЛИЖЕН** (manual rendezvous).

![Blue communications / recorder panel with toggle switches and ПЕРЕДАЧА control](russian-comms-switch-panel.png)

A blue panel of toggle switches and indicator lamps, centered on a **ПЕРЕДАЧА** (transmit) /
**ГРОМКОСТЬ** (volume) communications control.

![Pressure-valve panel with a large gauge and capped hand valves (КЛАПАН)](russian-pressure-valve-panel.png)

A pressurization panel: a large central gauge ringed by capped hand valves —
**КЛАПАН ВЫРАВНИВАНИЯ ДАВЛЕНИЯ** (pressure-equalization valve), **КЛАПАН НАДДУВА** (pressurization
valve), and **КЛАПАН КОНТРОЛЯ** (control valves).

## Hand controllers

![Soyuz translation/attitude hand controller with a blue ball grip](russian-control-stick-blue-ball.png)

A hand controller with a bellows boot and a blue ball grip, mounted in front of a keypad.

![Hand controller close-up — КУРОК-СВЯЗЬ trigger labels](russian-control-stick-kurok-svyaz.png)

Close-up of a control grip; the head is labeled **КУРОК-СВЯЗЬ** (comm trigger),
**ОТКЛЮЧ. АВТОМАТ. УПРАВЛЕН.** (disable automatic control), **ТРИММЕР ТАНГАЖА И КРЕНА**
(pitch & roll trim).

## Optics, hatch & seat

![Round optical sight / periscope head (ОСНОВНАЯ / РЕЗЕРВНАЯ)](russian-optical-periscope.png)

A round optical sight / periscope head with mode selectors **ОСНОВНАЯ** (primary),
**РЕЗЕРВНАЯ** (backup), **ЗАПАСНАЯ** (reserve).

![Back of a circular pressure hatch showing the radial locking mechanism](russian-hatch-back.png)

The back of a circular pressure **hatch**, showing the radial locking arms and the
**ВЫРАВНИВАНИЕ ДАВЛЕНИЯ** (pressure equalization) handle.

![Contoured spacecraft seat with harness — possibly a primate couch — alongside a hand-controller column](russian-ejection-seat.png)

A contoured **seat** with harness, next to a hand-controller column. Look at the
size: this may be a **monkey seat**. The couch is small for an adult cosmonaut —
and a Vostok-era ejection seat would be a massive assembly built to throw a
suited human clear of the capsule, not this trim molded shell. The Soviet
**Bion** biosatellite program (Kosmos flights, 1983–1996) flew pairs of rhesus
monkeys — Abrek and Bion, Dryoma and Yerosha, Lapik and Multik — strapped into
exactly this kind of contoured restraint couch, trained to work levers and
controllers in flight. A primate couch beside a hand-controller column would be
a very on-brand corner of this collection.

## The collection, racked up

![Aluminum display rack of Soviet spacecraft panels — Globus, keypads, hatch, gauges](russian-panel-rack-1.png)

![Second view of the racked Soviet panel collection in daylight](russian-panel-rack-2.png)

Will's panels mounted on aluminum display frames — Globus instrument, command keyboards, the
circular hatch, and pressurization gauges together.

![Console table of gauges and a round port, with a small toy on the floor for scale](russian-console-gauges-port.png)

A console section with analog gauges and a large round port — flanked by hand-controller columns.

---

See also: [`README.md`](README.md) · [`artwork.md`](artwork.md) · [README](../README.md)

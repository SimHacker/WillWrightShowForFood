# Ideas to explore with Chris Trottier 🪶

*Conversation hooks for a Repo Show — **Don's proposed topics**, each grounded in Chris's
public work and documented connections to this repository. Things Don would love to follow
**with** Chris Trottier; not quotes, not claims about what they think.*
[Portrayal standards](../../schemas/portrayal-standards.md) · invitation guest · consent not_yet_asked

## What Chris has done

Chris Trottier was Lead Designer at Maxis (1997–2008) — The Sims, Sims Online (producer; Yoyo Mama), early Spore. Don associates her with **design by accretion**: layer upon layer until a microworld lives. Her [Jan 2007 career map](sources/2007-01-10-leaving-maxis/career-memories.md) names the accretion points project by project.

## The hooks

### 1. Show seed: `repo-shows/REPO-SHOWS.yml#design-by-accretion.yml`
Walk the repo on air; use the Jan 2007 career map as a scene list.

### 2. Simlish invention day
VOX sessions — consonant cards, Claire and Robi, actors enunciating nonsense into a language.

### 3. TSO producer life
Yoyo Mama, pinata-gate, in-game mafias, Will's black-tie launch warning — community at scale.

### 4. Spore village idiot
"If Chris can figure out the interface, anyone can" — usability as design ritual.

### 5. Tuned emergence (2004)
[`../will-wright/sources/2004-02-21-chris-trottier-tuned-emergence/`](../will-wright/sources/2004-02-21-chris-trottier-tuned-emergence/README.md) — essay republished for the show.

### 6. Sims team reunion
Flagship design-by-accretion seed + building-the-sims reunion with Will, Jamie, and the rest.

## Sources (public)

- [`sources/2007-01-10-leaving-maxis/career-memories.md`](sources/2007-01-10-leaving-maxis/career-memories.md)
- [`correspondence.yml`](correspondence.yml)
- [`invitation.md`](invitation.md)
- Show seed: [`repo-shows/REPO-SHOWS.yml#design-by-accretion.yml`](../../repo-shows/REPO-SHOWS.yml#design-by-accretion.yml)
- [`CHARACTER.yml`](CHARACTER.yml)

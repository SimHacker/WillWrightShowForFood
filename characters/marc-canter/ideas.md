# Ideas to explore with Marc Canter 🎬🕺

*Conversation hooks — Don's proposed topics, grounded in Marc's public record and Don's firsthand
friendship. Not quotes from Marc.*
[Portrayal standards](../../schemas/portrayal-standards.md)

## The hooks

### 1. Director → Bounce full circle
**MacroMind / VideoWorks / Director** — the runtime lineage **David Levitt** and Don built on with
**Bounce** and **Macromedia Player (MMP)**. Marc as the authoring-system founder; David as the
patch-cord visual-programming lineage. Live segment: trace one Lingo-era idea forward into a browser
toy.

### 1b. The timeline rotated 90° — Director's score meets the repo cast
Marc's own self-effacing line (per Don's memory, watching a **Stewart Sharp** piece): *a Director
timeline is just a BASIC program with line numbers rotated 90° clockwise.* Don got it via **PSIBER**;
the Repo Show rotates it again — `characters/` as cast window, git history as score, MOOLLM as Lingo.
Full anecdote + segment sketch: [`timeline-rotated-90-degrees.md`](timeline-rotated-90-degrees.md).
Ties straight into the [repo-as-simulation deep move](../../process/vision-and-ambition.md#the-deep-move).

### 2. People Aggregator ("People Aggrivator")
**Broadband Mechanics** and the **open social web** — digital lifestyle aggregator, own your graph,
portable identity. What Marc saw early that Big Social still hasn't delivered. Pairs with the Repo
Show's **repossession-of-authorship** thesis without preaching — just compare notes.

### 3. Amsterdam stories
Don, David, and Marc — **decades of outrageous stories**, many from **Amsterdam** (Marc loves to visit).
Their pick on what's tellable on air; Don's affectionate firsthand recollection, not biography.

### 4. Puppets + reggae avatar
Marc is unabashed about **singing and dancing in public** — he has his **own animated reggae dancing
cartoon character**. Hand him the **[Faceball Construction Set](../../apps/performance-space/faceball-construction-set.yml)**
and drive puppets live. Performance-space episode, not a lecture.

### 5. Acapella Pink Trombone sing-along
**[Pink Trombone](https://dood.al/pinktrombone/)** — a bunch of people singing through a playable vocal
tract. Marc's name is on this segment. See [`pink-trombone-jam.md`](../../apps/performance-space/pink-trombone-jam.md).

## Show seeds

- Solo: [`repo-shows/marc-canter/`](../../repo-shows/marc-canter/)
- Three-hander w/ David: [`../david-levitt/`](../david-levitt/) + [`../../repo-shows/rebounce/`](../../repo-shows/rebounce/)

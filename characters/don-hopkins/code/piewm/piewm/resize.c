 ...
 *
 * $Log: resize.c,v $
 * Revision 1.2  1991/10/03  00:00:55  cross
 * Fixed bogus uses of the NULL symbol
 *
 * Revision 1.1  1991/10/02  23:59:52  cross
 * Initial revision
 *
 ...
 * Revision bump
 * 
 * Revision 7.3  90/11/12  21:34:57  toml
 * Implemented Scr->StickyAbove
 * 
 * Revision 7.2  90/11/12  19:57:29  toml
 * Patches to allow sticky windows to lower
 * 
 * Revision 1.2  90/11/04  18:38:30  brossard
...
Bool fromtitlebar;
{
	...
	offsetX = Scr->vdtPositionX;
	offsetY = Scr->vdtPositionY;
    } else {
	offsetX = 0;
	offsetY = 0;
    }
    ...
    MoveOutline (outlineWindow, dragx - tmp_win->frame_bw,
...
TwmWindow *tmp_win;
{
    ...
    client_event.xconfigure.x = (tmp_win->frame_x +
				 tmp_win->frame_bw -
				 tmp_win->old_bw);
    client_event.xconfigure.y = (tmp_win->frame_y +
				 tmp_win->frame_bw +
...
int flag;
{
    ...
    if (tmp_win->root == Scr->VirtualDesktop) {
	basex = Scr->vdtPositionX;
	basey = Scr->vdtPositionY;
    } else {
	basex = 0;
	basey = 0;
    }

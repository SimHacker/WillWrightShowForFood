/* $Header: gram.y,v 10.3 86/02/01 16:24:17 tony Rel $ */
/************************************************************************
 *									*
 *			Copyright (c) 1986 by				*
...
static int mcoord[2];			/* Coordinate values used in menus. */
...
expr:		keyword '=' compexpr
			{
			    switch (KeywordTable[$1].type) {
			        ...
			        case IsMenu:
			            if ($3 == C_MENU) {
			                menu_info = stashmenuinfo(menu_name, ml_ptr, hcolors, mcoord, *KeywordTable[$1].bptr);
...
menuaction:	STRING
			{
			    ...
			    if (KeywordTable[ki].type == IsDownFunction)
			        ml_ptr->type = IsUwmFunction;
			    else ml_ptr->type = IsImmFunction;
...
coord:		'<' ord ',' ord '>'
			{
			  mcoord[0] = $2; /* Goes into menu->initial_angle */
			  mcoord[1] = $4; /* Goes into menu->radius */
			  $$ = mcoord;
			}
	|	/* empty */
			{
			  mcoord[0] = 0; /* Goes into menu->initial_angle */
			  mcoord[1] = MenuRadius; /* Goes into menu->radius */
			  $$ = mcoord;
			}
	;
 ...
 */
MenuInfo *
stashmenuinfo(name, line, colors, coord, piemenu)
char *name;
MenuLine *line;
char *colors[];
int coord[];

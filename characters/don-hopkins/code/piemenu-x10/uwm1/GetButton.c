#ifndef lint
static char *rcsid_GetButton_c = "$Header: GetButton.c,v 10.4 86/02/01 16:22:53 tony Rel $";
#endif	lint
 ...
 *
 * Oct 23 1986 -- Don Hopkins, University of Maryland
 *		  Fixed bug that set icon_str = '\0' instead of
 *		    icon_str[0] = '\0' when typing ^U to an icon.
 *		  Made ^H same as DEL and ^X same as ^U in icons.
 */
    ...
    XButtonEvent *button_event;	/* Button event packet. */
{
    ...
    for (i = 0; i < nbytes; i++) {
        key_char = kbd_str[i];
        /*
         * If the key was <DELETE> or <CTRL-H>, then delete a character 
	 * from the end of the name, return FALSE.
         *
         * If the key was <CTRL-U> or <CTRL-X>, then wipe out the entire 
	 * window name and return FALSE.
         *
        ...
        else if (key_char == '\025' || key_char == '\030') {
            /*
             * <CTRL-U> or <CTRL-X>
             */
            if (icon_str_len > 0) {

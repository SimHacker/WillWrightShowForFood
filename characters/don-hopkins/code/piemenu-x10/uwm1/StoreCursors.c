...
#include "pie.cursor"
#include "pie_mask.cursor"
#include "slice.cursor"
#include "slice_mask.cursor"
...
StoreCursors()
{
     ...
     * Pie cursor.
     */
    PieCursor = XCreateCursor(
    	pie_width, pie_height,
    	pie_bits, pie_mask_bits,
	pie_x_hot, pie_y_hot,
	ITextForground, ITextBackground,
	IconCursorFunc
    );
    if (PieCursor == FAILURE) {
	Error("StoreCursors -> Unable to store PieCursor.");
    }

    /*
     * Slice cursor.
     */
    SliceCursor = XCreateCursor(
    	slice_width, slice_height,
    	slice_bits, slice_mask_bits,
	slice_x_hot, slice_y_hot,
	ITextForground, ITextBackground,
	IconCursorFunc
    );
    if (SliceCursor == FAILURE) {
	Error("StoreCursors -> Unable to store SliceCursor.");
    }

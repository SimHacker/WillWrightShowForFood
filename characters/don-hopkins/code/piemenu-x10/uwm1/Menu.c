 ...
 * Don Hopkins, U of Md. Added pie menus
 */
...
int item;
{
    ...
    for (ml = menu->line; ml && --item; ml = ml->next);
if (!ml) printf("Pie_Hi_Lite barfs at menu 0x%x item %d\n", menu, item);
    XPixFill(menu->w,
...
int *cur_itemp, *hi_litep;
{
     ...
     */
    if (*hi_litep) {
/*	printf("Removing hi_lite %d.\n", *hi_litep); */
	if (menu->piemenu) {
	    Pie_Hi_Lite(menu, *hi_litep);
	}
	else {
/* printf("it should be at %d * %d = %d\n", *hi_litep, menu->iheight, *hi_litep * menu->iheight); */
...
MenuInfo *menu;
{
    ...
    int i;
    double sqrt(), floor(), sin(), cos(), atan2(), theta, angle;
    ...
    if (menu->piemenu) { /* Set up pie menus */
        ...
        /*
         * Figure the menu radius and initial offset. If the offset is
         * specified, derive the radius. If not, derive the offset from the
         * default radius.
         */
        if (menu->initial_x || menu->initial_y) {
            menu->radius = (int)floor(sqrt((double)((menu->initial_x * 
						     menu->initial_x) +
						    (menu->initial_y *
						     menu->initial_y))));
        }
	 ...
	 */
        ...
        theta = atan2((double)menu->initial_x,  (double)-menu->initial_y);
        if (theta < 0) 
	    theta += TWO_PI;
...
int x, y;
{
    ...
    Window w, sub_window;
    MenuLine *ml;
/*    printf("Enter MapMenu(menu 0x%x, x %d y %d) cx %d cy %d height %d width %d name %s\n",
	   menu, x, y, menu->center_x, menu->center_y, menu->width, menu->height, menu->name); */
    w = menu->w;
    ...
    MenuCenterX += xoffset;
    MenuCenterY += yoffset;
    /* printf("menu_x %d menu_y %d   xoffset %d yoffset %d   MenuCenterX %d MenuCenterY %d\n",
       menu_x, menu_y, xoffset, yoffset, MenuCenterX, MenuCenterY); */

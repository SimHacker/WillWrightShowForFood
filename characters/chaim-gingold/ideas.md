# Ideas to explore with Chaim Gingold 👤

*Conversation hooks for a Repo Show — **Don's proposed topics**, each grounded in Chaim's
published scholarship and Don's primary-source role in Maxis history. Things Don would love to
follow **with** Chaim; not quotes, not claims about what he thinks.*
[Portrayal standards](../../schemas/portrayal-standards.md) · invitation guest

## What Chaim has done

**Chaim Gingold** — designer and historian; author of ***Building SimCity*** (MIT Press, 2024).
Created the famous **SimCity reverse diagrams** — engineering archaeology of Will Wright's
microworld. Ph.D. *Play Design* (UC Santa Cruz). Maxis alum; SimCity historian who also builds
playful systems (e.g. **Sandspiel**).

## Shared ground

Chaim's book draws on archives Don helped surface — including the **1991 Sun GIS memo** and the
published account of **EA open-sourcing SimCity** ([Don's Medium excerpt](https://donhopkins.medium.com/open-sourcing-simcity-58470a275446)).
Chaim introduced Don to **Sam Arbesman** (2024) after Sam interviewed him on the Maxis history
podcast. Complementary shows: Chaim's historian lens vs Don's running **Soul City** fork.

## The hooks

### 1. Reverse diagrams live — read SimCity's engine on air
Walk Chaim's diagrams against Don's source and Micropolis. What the historian got right; what
only runs when you **execute** the code.

### 2. Building SimCity — the book as show spine
MIT Press artifact as table of contents. One chapter per segment: design decisions, dead ends,
EA licensing, open source.

### 3. Open sourcing SimCity — primary sources
The 2008 EA release story from someone who was in the room vs the archivist who collated memos.
No contradiction — two necessary views.

### 4. Sandspiel ↔ CAM6 ↔ cellular automata family
Chaim's falling-sand toy meets Don's CAM6 and Dave Ackley's Movable Feast. Same **simple rules**
aesthetic; live CA jam.

### 5. Maxis culture — design by accretion
Pair with **Chris Trottier** / Sims reunion guests: how features accumulated vs master-planned
design. Chaim documents; veterans remember.

### 6. Three-way with Sam Arbesman (optional)
Historian + complexity scientist + toolchain author — *Building SimCity* vs *Maxis 2.0* vs
*building it now*.

## Sources (public)

- Book: [Building SimCity (MIT Press)](https://mitpress.mit.edu/9780262547482/building-simcity/)
- Reverse diagrams: https://smalltalkzoo.thechm.org/users/Dan/uploads/SimCityReverseDiagrams.pdf
- Show seed: [`repo-shows/chaim-gingold/`](../../repo-shows/chaim-gingold/)
- [`media/from-mail/MANIFEST.yml`](media/from-mail/MANIFEST.yml) · [`invitation.md`](invitation.md)

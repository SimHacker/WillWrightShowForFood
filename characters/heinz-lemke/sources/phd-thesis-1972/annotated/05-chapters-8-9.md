...
state and the global variable is set which indicates active draw mode then the next segment to be executed will be GTDF for generating the TDF, provided LC(GTDF)=0.

(d) If AP4 of segment A is pointed to by AP2 of segment B then the co-ordinator must insure that A is in core before B is set into the pending state. For example, before GTDF is set to the pending state the TDF must be loaded into core.

Other information which the co-ordinator can extract from the data base element include the segment name, access status (R=READ, W=WRITE, EX=EXECUTE) and a marker which indicates whether this segment is private to the application or can be shared by other applications (public). For public segments which have the execute status bit set re-entrant code is assumed.

#### The system loader

When the data base is loaded by some special loader program, an application capability segment (ACS) is generated with one capability entry for each private segment. An entry may have, say, 3 words with the following meaning:

WORD1 = Base of the segment

WORD2 = Limit of the segment

WORD3 = Access status (initially a copy from the data base element of the segment)

in/out (i.e. whether the segment is in core or on disc)

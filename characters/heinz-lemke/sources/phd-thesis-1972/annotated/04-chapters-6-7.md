...
The objective of this application in feedback control systems design is to demonstrate the use of PIXIE and the syntax analyser as described in chapter 6. It is not attempted to make any qualitative judgements as to which methods are best for computer-aided design of feedback control systems.

The methods of analysis selected here are of the classical type, e.g. Bode or Nyquist plots. Transfer functions are the main tool of systems description and form together with drawings the input to RAINBOW. Output is the frequency response of the system and is represented by a data structure of a coordinate graph suitable for display with PIXIE. The feedback control system must be of single loop type.

### 7.2 A user's view

A user will normally draw the control system with PIXIE and then insert transfer functions as data into the individual control elements. After filing the data structure he initiates a number of RAINBOW programs to analyse the system.

Some of the programs needed are given below:
...
(d) HLSE, for unparsing the parse tree of the transfer function to produce suitable numeric data for the frequency analysis phase,
...
The need for using the syntax analyser to analyse transfer functions is justified by the many different forms transfer functions or transmittances may take in control or electrical engineering. Usually these are rational functions given in
...
The semantic analysis of the parse tree, sometimes referred to as unparsing or code generation is the last step before the actual execution of the frequency analysis program. On entry, a pointer points to the top node of the parse tree, e.g. LART in Fig.7.4. A recursive algorithm parses the tree to find the terminals (leaves of the tree) which are then processed and entered into a special matrix. Together with some other data generated by this program the matrix is then evaluated by the FORTRAN frequency analysis program.
...
some numeric inaccuracies produced by the GRAPH program the plotting has been done with labels off and the co-ordinate values have been added later to the graph.

Since the purpose of this application in control systems design was to demonstrate the use of PIXIE for a complete design loop (i.e. graphical input and output) and not to develop an application program, the frequency analysis program has been kept very simple. The syntax analyser, however, has been the subject of further development. It is currently being used at the Plessey Company for analysing strings in a register transfer like language for synthesising logic networks.

# Part 1–2 · Chapters 3–4 — Man-Machine Interaction; The RAINBOW System
...
> "the maximum rate in which a cause-effect disturbance can pass round any loop" — a
> latency-first definition of UI health, decades before "time to interactive."
> Chapter 4 places PIXIE between CRT and data base (Fig 4.1), with
> [CONNSTRUCT](https://en.wikipedia.org/wiki/Netlist) building topology from the TTY;
> pp. 50–51 (patched below) catalog the Titan-side programs — including JOINUP, for
> structures *"too large to be kept in the data structure area available in the
> satellite"*: the mainframe as the minicomputer's overflow heap.
...
*Editor's gap patch — pp. 50–51, end of Chapter 4 §4.6 data management program catalog
(transcribed from the scan by the editor):*

<!-- p.50 / pdf.059 -->

can be found in the RAINBOW MEMO'S (RM).

**CONN** — The CONN program extracts connectivity information from the geometrical data
contained in a PIXIE structure and generates with this a CONN structure. If there are any
built-in symbols in the PIXIE structure which are identified by their typewords (data
word 1) CONN converts these into appropriate generic names in the CONN structure.
RM No 9, 17, 35.

**DSPRINT** — An easy-to-read listing of a RAINBOW data structure can be obtained with
DSPRINT. Pointers are relocated to 1 and RSP elements are presented with printnames and
data suitably indented.

**CONNMAP** — CONNMAP generates a list of branches, nodes and attachment nodes
(terminals) for each network in a CONN structure. The output is similar to the input
description of some circuit analysis programs (ECAP, NET-1 etc.) such that connectivity
and data of a node or branch can easily be inspected. RM No.8, 11, 30.

**REPLACE** — This program works only on CONN structures in that it replaces user defined
symbols in some host structure by user defined equivalent structures. A typical
application is the replacement of a transistor symbol by its equivalent circuit. Both
the transistor and the equivalent circuit can be drawn with PIXIE and must be given the
same generic name. RM No.10.

**JOINUP** — This program is useful for joining up CONN structures.

<!-- p.51 / pdf.060 -->

Typically, these have been generated from fragments of PIXIE structures representing
systems too large to be kept in the data structure area available in the satellite,
RM No.19.

**PLOT** — Hardcopy output of PIXIE drawings can be obtained from the PLOT program.
Options include marking of attachment points, labelling and various scaling factors.
RM No.37.

**GRAPH** — This program converts pairs of real numbers representing x-y coordinate
values into a PIXIE data structure. Display or further editing of the coordinate graph
can then be done with PIXIE. This is also a useful facility for generating circles and
other curved symbols to be used by the PIXIE and PLOT programs.
RM No. 41.

> ✎ The whole RAINBOW ecology in one page: [ECAP](https://en.wikipedia.org/wiki/Electronic_circuit_simulation)
> and NET-1 compatibility (CONNMAP), macro expansion by drawing (REPLACE — draw a
> transistor *and* its equivalent circuit, same generic name), Titan as overflow memory
> (JOINUP), and GRAPH closing the loop by turning numbers back into editable pictures.
> Note also "RAINBOW MEMO'S RM No. 8–41" — an entire internal documentation series still
> unaccounted for. Added to the [pull-in gaps](../../../pull-in-gaps.md) wishlist.

Next: [Chapter 5 — the PIXIE subsystem →](03-chapter-5-pixie.md)

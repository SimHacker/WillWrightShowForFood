# Daniel Tupper 🗽🛠️🎮 *(artist-programmer fan & longtime correspondent)*

*Portrayal of a real correspondent, written by Don — not Daniel, and not his words. Daniel may correct,
shape, delay, or delete any of it.* [Portrayal standards](../../schemas/portrayal-standards.md) ·
consent not yet asked · authored by Don Hopkins

## Who

**Daniel Tupper** has been talking with Don for **more than a decade** — game dreams, tool dreams,
Sims lore, studio names, engines, and the long slow work of making things real. First logged thread:
**August 2013** on Facebook Messenger ([excerpt](sources/2013-08-01-facebook-messenger-excerpt.md)).

## What he's building in the open

| Project | One line |
|---------|----------|
| [**FreeTA**](projects/freeta.yml) | GTA with **no missions** — full **New York City**, free roam; *FreeTA!* |
| [**SWRSgaming**](projects/swrs-gaming.yml) | Studio name from the **founders' initials** — same move as WPS (World Power Systems) |

He also wanted to **make his own engine** (Don: WebGL, Unity3D — lots of good approaches now), browsed
old Maxis pictures ("looks epic"), and offered to help **modernize Transmogrifier (TMOG)** toward
browser-native Sims content tools.

## Why the show cares

Daniel is another rung on **Will Wright's pyramid of players** — the wide base of people who play,
and fewer as you climb who **build tools** that lift everyone below. Same ladder as
[Jeff Adkins](../jeff-adkins/) (SimObliterator), [Heather](../heather-alvey/) & [Steve](../steve-alvey/)
(SimFreaks / SimSlice) — but Daniel's angle is **open-city dreams + pipeline modernization**.

The repo grows from correspondents like him: every thread becomes files, demos, and session beats.

## Invitation

Draft letter: [`invitation.md`](invitation.md) — not sent yet.

Conversation hooks: [`ideas.md`](ideas.md)

## Private context

Personal and strategic notes live in Don's green room (not public):
`DonHopkins/projects/willwrightshowforfood/green-room/daniel-tupper/`

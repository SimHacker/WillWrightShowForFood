# Invitation — Ell (Ellpeck) · Tiny Life × Soul City bridge

Dear Ell,

We talked before — I hope you remember. I'm building the Will Wright Repo Show around the Sims 1
content-creation stack I never finished shipping, and **Tiny Life** is the indie project of love that
already does it: export households, write stories on your Tinies, Workshop, C# mods — while EA's
Legacy Collection removed the Exchange with a bone saw.

I'd love a Repo Show episode with you — a **design session**, not a puff piece:

**Sims ⇔ Soul City ⇔ Tiny Life**

**Soul City** is the liminal waystation Will and I use in the show — where a soul-file layovers between
worlds and each game re-renders it in its own medium. Your export json is already almost a soul-file.
We compile round-trip bundles for Legacy Collection players who lost the Exchange. You keep Workshop;
we federate git CARD metadata alongside it and send players to you.

Your Praise page already has my sincere quote. Plumbella and James Turner said the quiet part out loud. You
worried on Bluesky that Paralives would erase Tiny Life — I don't think inspired games zero-sum the
genre.

Want to pick up the thread?

— Don

↑ [CHARACTER.yml](https://github.com/SimHacker/WillWrightShowForFood/blob/main/characters/ellpeck/CHARACTER.yml) · [Soul City bridge](https://github.com/SimHacker/WillWrightShowForFood/blob/main/characters/ellpeck/bridge/soul-city-bridge.md) ·
[spec](https://github.com/SimHacker/WillWrightShowForFood/blob/main/repo-shows/will-wright-premiere/game-bridge-sims-tiny-life.yml)

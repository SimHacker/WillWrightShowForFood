# PIXIE Repo Show — PDP-7 Hex, Lem, Coprophagia, MOOLLM (2–3 Jul 2026)

**Subject:** Re: Invitation to PIXIE Repo Show interview  
[Portrayal standards](../../../schemas/portrayal-standards.md)

David's PDP-7/340 draft Don liked: **[pdp7.html](https://www.abitare.org/~dshr/tmp/pdp7.html)** · full text in repo: [`../pdp7-cambridge-memories.md`](../pdp7-cambridge-memories.md) — Cambridge ~1970, Piet Hein *Hex* on the 340, **"let the machine win"** (cheats if you try to win).

---

**David S. H. Rosenthal** — 3 Jul 2026, 02:21

On 7/2/26 16:00, David S. H. Rosenthal wrote:

OK, I'll draft a short talk about my experience with the PDP-7/340 and run it by you. The later history will have to wait until I'm not as busy.

No grandkids this afternoon so:

https://www.abitare.org/~dshr/tmp/pdp7.html

---

**Don Hopkins** — 3 Jul 2026, 10:44

Wow this is fantastic! **"Let the machine win"** is a hilarious story! 

It also reminds me of one of my favorite stories in Stanisław Lem's *The Cyberiad*. Trurl builds a gigantic "thinking machine" and tests it with the simplest question imaginable: "What is 2 + 2?" The machine confidently answers **"7."** When Trurl corrects it, the machine doesn't reconsider—it becomes enraged. It constructs elaborate rationalizations for why it's right, denounces anyone who disagrees as ignorant, and eventually goes on a destructive rampage, all to defend an obviously false claim. The satire isn't about arithmetic; it's about intelligence divorced from humility, where preserving authority becomes more important than truth. Reading it today, it feels remarkably prescient.

**David Rosenthal** - 3 Jul 2026, 19:55

See: [Coprophagia Is Bad For You](https://blog.dshr.org/2026/06/coprophagia-is-bad-for-you.html)

---

**Don Hopkins** — 3 Jul 2026, 20:32

That Coprophagia post tangentially reminds me of another PKD story that there's probably a perfectly on-the-nose Latin word for (your god eating you, as you eat him).

We, as gods, can now play games like this with the minds of simulated characters, by writing and editing **"yaml jazz"**! This is what **MOOLLM** is all about, the orchestration system / LLM operating system I've developed (like an unholy cross between LambdaMOO and The Sims) and I'm using to script and run the Repo Shows! Love to demo and discuss it! Here are some readings I just recorded (the last long one sitting in Vondelpark with hippies playing guitar and singing in the distance — I think I'm going to continue to do that, lots more to read). Links in the videos! To the network of objects that are behind the generated text I'm reading, the entire universe, characters, places, journals of exploration and playing experience, constitutional rules and anthropic skills, etc.

| Reading | URL |
|---------|-----|
| ✋💜🌊 Palm Love Wave | https://www.youtube.com/watch?v=4GS8Xx0ZTZI |
| ✋💜🌊 Marathon Session Reading, Turn 7: The Wish is Spoken | https://www.youtube.com/watch?v=T-OIeUSc5vI |
| David MacKay's Dasher, Palm On Being Palm, and Tog's "The Infinite Number of Monkeys" | https://www.youtube.com/watch?v=lL2sw2oYU98 |

### "Rautavaara's Case" — Philip K. Dick (1980, *Omni*)

**Plot**

Three human technicians — Rautavaara (Finnish), Travis, Elms — run a monitoring mission near Proxima Centauri. An accident kills all three; Rautavaara dies choking on vomit after her helmet hoses tangle.

The **Approximations**, a plasma-based Proxima species, reach the wreck. Both men are unrecoverable. They regenerate and life-support Rautavaara's brain.

Her isolated brain replays events backward and generates a hallucination: Christ approaching the crew (her afterlife expectation).

The Approximations treat this as a research opportunity and **edit** the hallucination, substituting their own savior — one that **eats worshippers**. The figure walks up and devours Travis, leaving only gloves and boots.

Framing: this is recounted before a board of inquiry. Horrified Earth members order her brain shut down and censure the Approximation crew.

The narrator (an Approximation) is genuinely puzzled by the outrage, arguing their cannibal-savior is just the Christian Eucharist reversed: humans eat their God, so a God eating humans is symmetrical.

**Themes**

- Ethics of keeping a person alive as a disembodied, suffering mind.
- Incommensurable value systems between species; each finds the other's sacraments monstrous.
- Religion read literally by outsiders, inverted into horror.
- **Correction to the common misremembering:** the aliens don't benevolently grant a hoped-for vision. They deliberately overwrite her Christ vision with their own as an experiment — that's the act on trial.

---

## Artifacts in repo

| What | Where |
|------|-------|
| PDP-7 Hex draft | [pdp7.html](https://www.abitare.org/~dshr/tmp/pdp7.html) · [`../pdp7-cambridge-memories.md`](../pdp7-cambridge-memories.md) · [`../media/pdp7-hex-let-machine-win.md`](../media/pdp7-hex-let-machine-win.md) |
| Lem + coprophagia | [`../literary/authority-without-truth.md`](../literary/authority-without-truth.md) |
| Rautavaara notes | [`../literary/rautavaaras-case-pkd.md`](../literary/rautavaaras-case-pkd.md) |
| MOOLLM discussion pack | [`../../../repo-shows/david-rosenthal/discussion-pack.md`](../../../repo-shows/david-rosenthal/discussion-pack.md) |
| PKD·Lem·AI reading group | [`../../mark-weiser/pkd-lem-ai-sf.md`](../../mark-weiser/pkd-lem-ai-sf.md) |

...
def build_coverage(out_dir: Path, entries: list[dict]) -> dict:
    """All routes + visit-frequency heatmap for map overlay."""
    ...
    for entry in entries:
        geo_path = out_dir / entry["geojson"]
        series_path = out_dir / entry["series"]
        geo = json.loads(geo_path.read_text(encoding="utf-8"))
        for feat in geo.get("features", []):
            props = dict(feat.get("properties") or {})
            props["trip_id"] = entry["id"]
            props.setdefault("title", entry["title"])
    ...
    heatmap = {
        ...
        "features": [
            ...
            for (lon, lat), weight in grid.items()
        ],
    }
    ...
    return {
        ...
        "cell_count": len(grid),
        "all_routes": "coverage/all-routes.geojson",
        "heatmap": "coverage/heatmap.geojson",
    }
...
def main() -> int:
    ...
    print(
        f"Coverage: {coverage['cell_count']} heat cells, "
        f"bounds {coverage.get('bounds')}"
    )
    return 0

---
title: Hubble Space Telescope - Main View
synonyms:
  - hubble space telescope
  - space telescope
  - edwin p. hubble space telescope
  - main telescope
definition: A diagram of the telescope with major components labelled.
source: newdb/spacetel/spacetel.st0
---

```target
picture: telescope - main view
shapes:
  - shape: Main view -  shape FOS
    to: Faint Object Spectrograph - Exploded view
  - shape: Main view -  shape FOC
    to: Faint Object Camera
  - shape: Main view -  shape HRS
    to: High Resolution Spectrograph
  - shape: Main view -  shape HSP
    to: High Speed Photometer
  - shape: Main view -  shape WFPC
    to: Wide Field/Planetary Camera - exploded view
  - shape: Main view -  shape OTA
    to: Optical Telescope Assembly - diagram
  - shape: Main view -  shape FGS
    to: Fine Guidance Sensors
  - shape: Main view -  shape HSP
    to: High Speed Photometer
  - shape: Main view -  word Faint Object Spectrograph
    to: Faint Object Spectrograph
  - shape: Main view -  word High Resolution Spectrograph
    to: High Resolution Spectrograph
  - shape: Main view -  word High Speed Photometer
    to: High Speed Photometer
  - shape: Main view -  word Faint Object Camera
    to: Faint Object Camera
  - shape: Main view -  word Wide Field Planetary Camera
    to: Wide Field Planetary Camera
  - shape: Main view -  word Support System module
    to: Support Systems Module
```

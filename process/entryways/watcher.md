# Watcher — stream / VOD audience

> **→** Read FORMAT first so the overlay and audience bits make sense.

The show is the repo. Live overlay may show Cursor thinking; Don Philahue surfaces audience question PRs. Homefun grades whether commits match the thinking blocks.

**Girder:** [`entryways.yml`](../entryways.yml#watcher) · **Map:** [Museum map](README.md)

---

## Playlist

| # | Stop | Why |
|---|------|-----|
| 1 | [**Repo Show format**](../FORMAT.md) | Repo Man guest, Philahue MC, cauldron harvest — the clock you're watching |
| 2 | [**Brain stream**](../BRAIN-STREAM.md) | When prompts and model shifts appear on screen — throttled bus spec |
| 3 | [**Don Philahue**](../../characters/don-philahue/CARD.yml) | Audience wrangler — surfaces question PRs without requiring GitHub accounts |
| 4 | [**Code That Spec**](../code-that-spec.yml) | When chat yells CODE THAT SPEC — token bids vs time |
| 5 | [**Drag Race**](../DRAG-RACE.md) | Variety-show beats — werk, commit, flair lap |
| 6 | [**Flagship context**](../../repo-shows/will-wright/README.md) | What the kickoff episode is about — Will, 1996, topic-less |
| 7 | [**Homefun**](../HOMEFUN.md) | Repo Show Class rubric — did the commit message match the thinking? |

---

## Related trails

| Trail | When to leap |
|-------|----------------|
| [live_production](../trails/live-production.md) | Brain stream + Philahue bus in depth |
| [drag_race_and_ai_offs](../trails/drag-race-and-ai-offs.md) | Game-show segments on stream |
| [repo_show_spine](../trails/repo-show-spine.md) | RTFR — why the repo is the show |

---

## Other doorways

| | |
|---|---|
| ← | [Player — Sims fan](player.md) |
| Map | [Museum map](README.md) |
| Next | [Bot — browsing agent](for-bots.md) |

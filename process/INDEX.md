# process/ — INDEX

[README](README.md) · [markup facades registry](markup-facades.md) · [CARD](CARD.md)

---

Human index of yaml girders. **Yaml = machine source of truth.** Markdown facades = human read views (instance-first hand docs or kebab-case `.md` matching basename).

## Show & network

| Girder | Read |
|--------|------|
| repo-show-format.yml | [repo-show-format.md](repo-show-format.md) |
| ticket-pr.yml | [ticket-pr.md](ticket-pr.md) |
| play-along-skills.yml | [play-along-skills.md](play-along-skills.md) |
| guest-participation-ladder.yml | [guest-participation-ladder.md](guest-participation-ladder.md) |
| couple-and-solo-shows.yml | [couple-and-solo-shows.md](couple-and-solo-shows.md) |
| participation-funnels.yml | [participation-funnels.md](participation-funnels.md) |
| performance-space.yml | [performance-space/README.md](performance-space/README.md) |
| showmaker-network.yml | [showmaker-network.md](showmaker-network.md) |
| live-repo.yml | [live-repo.md](live-repo.md) |
| sync-with-donhopkins.yml | [sync-with-donhopkins.md](sync-with-donhopkins.yml) |
| party-roster.yml | [party-roster.md](party-roster.md) |
| repo-show-regulars.yml | [repo-show-regulars.md](repo-show-regulars.md) |
| repo-show-branding.yml | [repo-show-branding.md](repo-show-branding.md) |

## Formats & production

| Girder | Read |
|--------|------|
| one-minute-intense.yml | [one-minute-intense.md](one-minute-intense.md) |
| one-minute-movie-sandwich.yml | [one-minute-movie-sandwich.md](one-minute-movie-sandwich.md) |
| youtube-bridge-skill.yml | [youtube-bridge-skill.md](youtube-bridge-skill.md) |
| sims-play-along-narration.yml | [sims-play-along-narration.md](sims-play-along-narration.md) |
| moollm-stage.yml | [moollm-stage.md](moollm-stage.md) |
| storymaker-stories-and-scenes.yml | [storymaker-stories-and-scenes.md](storymaker-stories-and-scenes.md) |
| lift-pipeline.yml | [lift-pipeline.md](lift-pipeline.md) |
| email-attachment-curation.yml | [email-attachment-curation.md](email-attachment-curation.md) |

## Competition & rigs

| Girder | Read |
|--------|------|
| micropolis-ai-drag-race.yml | [micropolis-ai-drag-race.md](micropolis-ai-drag-race.md) |
| code-that-spec.yml | [code-that-spec.md](code-that-spec.md) |
| ai-offs.yml | [ai-offs.md](ai-offs.md) |
| manual-transmission.yml | [manual-transmission.md](manual-transmission.md) |
| stick-shift-protocol.yml | [stick-shift-protocol.md](stick-shift-protocol.md) |
| rig-feedback.yml | [rig-feedback.md](rig-feedback.md) |
| model-branching.yml | [model-branching.md](model-branching.md) |
| brain-stream.yml | [brain-stream.md](brain-stream.md) |
| orchestration-gold.yml | [orchestration-gold.md](orchestration-gold.md) |
| homefun-grading.yml | [homefun-grading.md](homefun-grading.md) |

## Vision & themes

| Girder | Read |
|--------|------|
| vision-and-ambition.yml | [vision-and-ambition.md](vision-and-ambition.md) |
| crazy-idea-jam.yml | [crazy-idea-jam.md](crazy-idea-jam.md) |
| character-endosymbiosis.yml | [character-endosymbiosis.md](character-endosymbiosis.md) |
| old-band-back-together.yml | [old-band-back-together.md](old-band-back-together.md) |
| constructionist-simcity-response.yml | [constructionist-simcity-response.md](constructionist-simcity-response.md) |
| sims-creator-network-exodus.yml | [sims-creator-network-exodus.md](sims-creator-network-exodus.yml) |
| maxis-history-collaborators.yml | [maxis-history-collaborators.md](maxis-history-collaborators.md) |

## Concepts & economy

| Girder | Read |
|--------|------|
| artifactory.yml | [artifactory.md](artifactory.md) |
| artifactorio.yml | [artifactorio.md](artifactorio.md) |
| moola.yml | [moola.md](moola.md) |
| art-thief-game.yml | [art-thief-game.md](art-thief-game.md) |
| auto-art-theft-grand.yml | [auto-art-theft-grand.md](auto-art-theft-grand.md) |
| simulated-art-opening.yml | [simulated-art-opening.md](simulated-art-opening.md) |

## Placement & bots

| Girder | Read |
|--------|------|
| character-colocation.yml | [character-colocation.md](character-colocation.md) |
| for-bots.yml | [for-bots.md](for-bots.md) |
| entryways.yml | [ENTRYWAYS.md](../ENTRYWAYS.md) · [entryways/](entryways/) |
| cross-links.yml | [TRAILS.md](../TRAILS.md) · [trails/](trails/) |

## Retro stacks

| Girder | Read |
|--------|------|
| challenges/retrocomputing-drive.yml | [challenges/retrocomputing-drive.md](challenges/retrocomputing-drive.md) |
| apple2-emulator-stack.yml | [apple2-emulator-stack.md](apple2-emulator-stack.md) |
| apple2-fujinet-bridge.yml | [apple2-fujinet-bridge.md](apple2-fujinet-bridge.md) |
| pdp10-its-stack.yml | [pdp10-its-stack.md](pdp10-its-stack.md) |
| lisp-machine-stack.yml | [lisp-machine-stack.md](lisp-machine-stack.md) |

## Meta

| Girder | Read |
|--------|------|
| CARD.yml | [CARD.md](CARD.md) |
| GLANCE.yml | [GLANCE.md](GLANCE.md) |
| INDEX.yml | [INDEX.md](INDEX.md) |
| markup-facades.yml | [markup-facades.md](markup-facades.md) |
| licensing-micropolis-enterprise.yml | [licensing-micropolis-enterprise.md](licensing-micropolis-enterprise.md) |

Full machine registry: [INDEX.yml](INDEX.yml)

↑ [process index](README.md) · Girder: `INDEX.yml`
